...
def main():
    logger.header("PROCESSANDO AUDIOS")
    
    # Lê os dados do Excel
    frente, verso, audios = ler_dados_excel()

    assert len(frente) == len(verso), "Os arquivos devem ter o mesmo numero de frases!"
    ...
    logger.info(f"Encontrados {len(arquivos_mp3)} arquivos de áudio")
    ...
    logger.info("Copiando arquivos para pasta do Anki...")
    arquivos_copiados = 0
        ...
        if linha.startswith("[sound:") and linha.endswith("]"):
            ...
            if os.path.exists(origem):
                if os.path.abspath(origem) != os.path.abspath(destino): 
                    ...
                    logger.info(f"Arquivo ja esta na pasta destino: {nome_arquivo}")
            else:
                logger.error(f"Audio nao encontrado: {nome_arquivo}")

    logger.success(f"Total de arquivos copiados: {arquivos_copiados}")
